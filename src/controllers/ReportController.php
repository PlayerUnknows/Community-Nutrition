...
class ReportController
{
    ...
    public function previewReport()
    {
        try {
            ...
            $heightData = $this->reportModel->getHeightData($startDate, $endDate);
            
            // Debug - log data counts and structure
            error_log("Preview Report - Type: $type, StartDate: $startDate, EndDate: $endDate");
            error_log("Data counts - Growth Trends: " . count($data) . 
                    ", Nutritional Status: " . (is_array($nutritionalStatus) ? count($nutritionalStatus) : 'not array') . 
                    ", Growth Stats By Gender: " . count($growthStatsByGender) . 
                    ", Gender Distribution: " . (is_array($genderDistribution) ? count($genderDistribution) : 'not array') .
                    ", Height Data: " . (is_array($heightData) ? count($heightData) : 'not array'));
            error_log("Nutritional Status structure: " . json_encode($nutritionalStatus));
            error_log("Height Data structure: " . json_encode($heightData));

            // Process data for charts
            $growthStatusData = [
                'Stunted' => (int)($nutritionalStatus['stunted'] ?? 0),
                'Normal' => (int)($nutritionalStatus['normal_height'] ?? 0),
                'Over' => (int)($nutritionalStatus['tall'] ?? 0)
            ];
            ...
            switch ($type) {
                case "growth-chart":
                    // Process gender distribution data for charts
                    $genderCounts = [
                        'Male' => 0,
                        'Female' => 0
                    ];
                ...
                case "growth-summary":
                    ...
                    $totalCount = ($nutritionalStatus['stunted'] ?? 0) + 
                                 ($nutritionalStatus['normal_height'] ?? 0) + 
                                 ($nutritionalStatus['tall'] ?? 0);
                    
                    $statuses = [
                        'Stunted' => (int)($nutritionalStatus['stunted'] ?? 0),
                        'Normal' => (int)($nutritionalStatus['normal_height'] ?? 0),
                        'Tall' => (int)($nutritionalStatus['tall'] ?? 0)
                    ];
                ...
                case "height-measurements":
                    error_log("Height measurements case triggered - Data count: " . (is_array($heightData) ? count($heightData) : 'not array'));
                    ...
                    if (is_array($heightData) && count($heightData) > 0) {
                        foreach ($heightData as $record) {
                            if (!isset($record['measurement_date']) || !isset($record['patient_name'])) {
                        ...
                        echo '<tr><td colspan="6" class="text-center">No height measurement data available for the selected date range.</td></tr>';
                    }
