$(document).ready(function() {
        ...
        paginatedData.forEach(function(user) {
            const row = `
                <tr>
                    ...
                    <td>${user.email}</td>
                    <td>
                        <span class="badge bg-${user.role === 'Administrator' ? 'primary' : user.role === 'Brgy Health Worker' ? 'success' : 'info'}">${user.role}</span>
                    </td>
                    <td>${user.created_at}</td>
                    <td>
                        <div class="d-flex gap-1">
                            <button class="btn btn-sm btn-outline-info view-user" data-id="${user.user_id}" title="View">
                                <i class="far fa-eye"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-primary edit-user" data-id="${user.user_id}" title="Edit">
                                <i class="far fa-edit"></i>
                            </button>
                            <button class="btn btn-sm btn-outline-danger delete-user" data-id="${user.user_id}" title="Delete">
    ...
    function updatePagination(totalItems) {
        ...
        $('#usersPrevPage').parent().toggleClass('disabled', currentPage === 1);
        $('#usersNextPage').parent().toggleClass('disabled', currentPage === totalPages);
    }
