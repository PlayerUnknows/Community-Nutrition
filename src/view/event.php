    ...
    <div class="container-fluid mt-4">
        <div class="row justify-content-center">
            <main class="col-md-11 col-lg-15 px-6">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-4 pb-3 mb-4 border-bottom bg-white px-3 shadow-sm" style="height: 50px;">
                ...
                <!-- Event List Table -->
                <div class="table-responsive bg-white rounded shadow-sm p-3" id="event" style="height: 400px; overflow-y: auto;">
                    <table class="table table-hover">
                        ...
                        <tbody class="align-middle">
                            ...
                            foreach ($events as $event) {
                                ...
                                echo "<td>
                                    <button class='btn btn-sm btn-outline-primary me-1' onclick='editEvent({$event['event_prikey']}, \"{$event['event_type']}\", \"{$event['event_name_created']}\", \"" . date('H:i', strtotime($event['event_time'])) . "\", \"" . date('Y-m-d', strtotime($event['event_date'])) . "\", \"{$event['event_place']}\")'>
                                        <i class='far fa-edit'></i>
                                    </button>
                                    <button class='btn btn-sm btn-outline-danger' onclick='confirmDeleteEvent({$event['event_prikey']})'>
                ...
                <script>
                                ...
                                events.forEach(event => {
                                    try {
                                        ...
                                        row.innerHTML = `
                                            ...
                                            <td>
                                                <button class='btn btn-sm btn-outline-primary me-1' onclick='editEvent(${event.event_prikey}, "${event.event_type || ''}", "${event.event_name_created || ''}", "${event.event_time ? event.event_time.substring(0, 5) : ''}", "${event.event_date || ''}")'>
                                                    <i class='far fa-edit'></i>
                                                </button>
                                                <button class='btn btn-sm btn-outline-danger' onclick='confirmDeleteEvent(${event.event_prikey})'>
                                                    <i class='fa fa-trash'></i>
                    ...
                    function editEvent(eventId, eventType, eventName, eventTime, eventDate) {
                        ...
                        document.getElementById('edit_event_prikey').value = eventId;
